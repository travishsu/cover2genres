...
acc = []
accuracies = []
best_threshold = np.zeros(pred_train.shape[1])
for i in range(pred_train.shape[1]):
    y_prob = np.array(pred_train[:,i])
    for j in threshold:
        y_pred = [1 if prob>=j else 0 for prob in y_prob]
        acc.append( matthews_corrcoef(train_y[:,i],y_pred))
    acc   = np.array(acc)
    index = np.where(acc==acc.max())
    accuracies.append(acc.max())
    best_threshold[i] = threshold[index[0][0]]
    acc = []
pred_y = np.array([[1 if pred_train[i,j]>=best_threshold[j] else 0 for j in range(train_y.shape[1])] for i in range(len(train_y))])

pred = model.predict(test_x)
p  = greater50percent(pred, best_threshold)
p_ = greater50percent(test_y, best_threshold)
for i in xrange(len(p)):
    print(i, p[i], p_[i])
print(hamming_loss(pred_y, train_y))
